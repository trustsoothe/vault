...
import { MsgClaimMorseAccount } from '../Cosmos/pocket/client/pocket/migration/tx'
...
export class PocketNetworkProtocolService
  ...
  async signMigrateMorseMessage({ privateKey, shannonAddress }: {
    privateKey: string;
    shannonAddress: string
  }) {
    const message = MsgClaimMorseAccount.fromPartial({
      shannonSigningAddress: shannonAddress,
      shannonDestAddress: shannonAddress,
      morsePublicKey: this.hexStringToByteArray(this.getPublicKeyFromPrivateKey(privateKey)),
    })

    const bytesToSign = MsgClaimMorseAccount.encode(message).finish()
