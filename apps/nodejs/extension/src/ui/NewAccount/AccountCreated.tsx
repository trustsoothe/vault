...
import { networkSymbolSelector, selectedChainByProtocolSelector } from '../../redux/selectors/network'
...
export default function AccountCreated({ account }: AccountCreatedProps) {
  ...
  const selectedChainByProtocol = useAppSelector(selectedChainByProtocolSelector)
  ...
  return (
    <Stack padding={2.4} spacing={1.6}>
      <Stack height={40} spacing={1} paddingX={1.4} paddingY={1.2} alignItems={'center'} borderRadius={'8px'}
             boxSizing={'border-box'}
             bgcolor={themeColors.successLight} direction={'row'}>
        <SuccessIcon/>
      ...
      <Summary
        rows={[
          {
            label: 'Name',
            value: <Stack direction={'row'} alignItems={'center'} spacing={0.7}>
      ...
      <Summary rows={[
        {
          label: 'Balance',
          value: loadingBalance ? <Skeleton variant={'rectangular'} width={100} height={20}/> : (
